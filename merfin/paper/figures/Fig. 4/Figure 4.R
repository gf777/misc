...
plot_lines <- function(df, stage, qv, dataset, title, y_title, steps) {
  ...
  plt <- ggplot(df, aes(x=stage, y=qv)) +
    ...
    ggtitle(title)+
    scale_color_manual(values=c("black","#21409A","#ED1C24"), breaks = c(2,3,4), labels = rev(c("Default", "Merfin", "Merfin + Default")), guide = guide_legend(reverse = TRUE))+
...
plot_bars <- function(df, stage, readout, title) {
  plt<-ggplot(df, aes_string(x="stage", y=readout)) +
    ...
    scale_fill_manual(values=c("#999999", "#ED1C24","#21409A"))+
    scale_color_manual(values=c("#999999", "#ED1C24","#21409A"))+
    ggtitle(title) +
...
#devSVG(file = "Figure 3.svg", width = 65, height = 50, bg = "white", fg = "black", onefile = TRUE, xmlHeader = TRUE)

png("Figure 4 main.png", width = 6500, height = 5000)
