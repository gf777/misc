...
parser.add_argument("--max-gap", dest="max_gap", type=int, default=10000, help="Max length of gap to fill")
...
def report_ctg(name):
    ...
    print("%s.0: %s\t%d\t%d\t.\t0\t+" % (name, name, start_map[name], end_map[name]))
...
def compatible(e, s):
    ...
    if e.reverse != s.reverse:
        return False
    if not e.reverse:
        if s.rstart > e.rend and s.rstart < e.rend + args.max_gap:
            print("Closing gap between %s and %s with %s+" % (e.qname, s.qname, e.rname), file=sys.stderr)
            print("%s.1: %s\t%d\t%d\t.\t0\t+" % (e.qname, e.rname, e.rend, s.rstart))
            return True
    else:
        if e.rstart > s.rend and e.rstart < s.rend + args.max_gap:
            print("Closing gap between %s and %s with %s-" % (e.qname, s.qname, e.rname), file=sys.stderr)
            print("%s.1: %s\t%d\t%d\t.\t0\t-" % (e.qname, s.rname, s.rend, e.rstart))
            return True
    return False
